...
@interface PhotoCell ()<UIScrollViewDelegate,UIGestureRecognizerDelegate>
...
- (void)setUpUI
{
    ...
    LoadingView *loadingView = [[LoadingView alloc] initWithFrame:self.contentView.bounds];
...
- (void)panGes:(UIPanGestureRecognizer *)panGes
{
    ...
    switch (panGes.state) {
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            CGFloat scale = ((effectDistance - (self.imageView.frame.origin.y - self.sourceFrame.origin.y)) / effectDistance) * 0.8;
            if (scale > 1) {
                scale = 1;
            }
...
- (void)setBrowser:(PhotoBrowser *)browser
{
    _browser = browser;
...
- (void)startCloseAnimation
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if (self.browser.animationType == AnimationTypeScale) {
